...
response = requests.get("https://api.github.com/repos/%s/%s/pulls?state=closed&page=1&per_page=10" % (owner, repos), headers={'Authorization': 'token %s' % (os.getenv("access_token"))})
...
with open("ouput.json", "w", encoding="utf-8") as f:
    json.dump(output_object, f, ensure_ascii=False, indent=4)
